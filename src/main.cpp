...
const string strMessageMagic = "child Signed Message:\n";
...
{
    // child: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nTargetTimespan = 30 * 60; // Child: 1 Day
static const int64 nTargetSpacing = 1 * 60; // Child: 10 Minutes
...
unsigned int static GetNextWorkRequired_V1(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // child: This fixes an issue where a 51% attack can change difficulty at will.
...
unsigned int static DarkGravityWave(const CBlockIndex* pindexLast, const CBlockHeader *pblock) {
    /* current difficulty formula, child - DarkGravity, written by Evan Duffield - evan@child.io */
    const CBlockIndex *BlockLastSolved = pindexLast;
    const CBlockIndex *BlockReading = pindexLast;
    const CBlockHeader *BlockCreating = pblock;
    BlockCreating = BlockCreating;
...
unsigned int static DarkGravityWave2(const CBlockIndex* pindexLast, const CBlockHeader *pblock) {
    /* current difficulty formula, child - DarkGravity v2, written by Evan Duffield - evan@child.io */
    const CBlockIndex *BlockLastSolved = pindexLast;
    const CBlockIndex *BlockReading = pindexLast;
    const CBlockHeader *BlockCreating = pblock;
    BlockCreating = BlockCreating;
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // child: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // child: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {        
        const char* pszTimestamp = "Save the children DCG";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
...
unsigned char pchMessageStart[4] = { 0xa3, 0xd5, 0xc2, 0xf9 }; // child: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("childMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("childMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static childMiner(CWallet *pwallet)
{
    printf("childMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("child-miner");
    ...
    try { while (true) {
        ...
        printf("Running childMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&childMiner, pwallet));
}
